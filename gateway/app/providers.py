...
def _upstreams() -> dict[str, str]:
    # k8s_events는 backward-compatible Loki log-signal alias다. Real Kubernetes
    # evidence는 provider="kubernetes"로 Kubernetes API에서 별도 조회한다.
...
def run_query(provider: str, query: str, start: datetime, end: datetime, limit: int | None) -> dict:
    ...
    if provider in ("loki", "k8s_events", "kubernetes_log_signal"):
        result = _loki(provider, query, start, end, limit)
        return _mark_kubernetes_log_signal(result) if provider in ("k8s_events", "kubernetes_log_signal") else result
    if provider == "tempo":
